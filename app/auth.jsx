/* Berg PC Platform — auth (login + register). Exports window.BergAuth */
const DSA = window.BergPCDesignSystem_eb13e3;
const { Button, Input, Field, Checkbox } = DSA;
...
function AuthScreen({ onAuthed }) {
  ...
  function submit(e) {
    ...
    const res = mode === "login"
      ? window.DB.login(form.email, form.password)
      : window.DB.register({ name: form.name, email: form.email, password: form.password, role: "client" });
    if (res.error) { setErr(res.error); return; }
    onAuthed(res.user);
  }

  const demo = [
    ["Administrator", "admin@bergpc.com", "admin", "shield"],
    ["Attorney", "geoff@bergpc.com", "lawyer", "scale"],
    ["Client", "jane@email.com", "client", "user-round"],
  ];
  function quick(email, password) { setMode("login"); setForm({ name: "", email, password }); setErr(""); }
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "var(--font-ui)", background: "var(--cream)" }}>
      ...
      <div style={{ flex: "1 1 52%", background: "var(--cream)", display: "flex", alignItems: "center", justifyContent: "center", padding: 32, overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: 384 }}>
          <img src={LOGO_REV_A} alt="Berg PC" style={{ height: 40, marginBottom: 30, filter: "invert(1) brightness(0.18)", display: "none" }} />
          ...
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <Button type="submit" variant="primary" fullWidth size="lg" iconRight={<Ico n="arrow-right" s={18} />}>{mode === "login" ? "Sign in" : "Register & continue"}</Button>
          </form>
          ...
          <div style={{ borderTop: "1px solid var(--border-subtle)", paddingTop: 18 }}>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--ink-400)", marginBottom: 11 }}>Demo accounts · one-click sign-in</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {demo.map(([role, email, pw, icon]) => (
                <button key={email} onClick={() => quick(email, pw)} type="button"
                  style={{ display: "flex", alignItems: "center", justifyContent: "space-between", border: "1px solid var(--border-default)", background: "var(--white)", borderRadius: "var(--radius-sm)", padding: "10px 13px", cursor: "pointer", fontFamily: "var(--font-ui)", transition: "border-color .12s, box-shadow .12s" }}
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--ink-900)"; e.currentTarget.style.boxShadow = "var(--shadow-sm)"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border-default)"; e.currentTarget.style.boxShadow = "none"; }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13.5, color: "var(--ink-800)", fontWeight: 500 }}>
                    <span style={{ width: 26, height: 26, borderRadius: "50%", background: "var(--stone-100)", color: "var(--stone-700)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}><Ico n={icon} s={14} /></span>
                    {role}
                  </span>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 11.5, color: "var(--ink-400)" }}>{email}</span>
                    <Ico n="arrow-right" s={14} c="var(--ink-300)" />
                  </span>
                </button>
              ))}
            </div>
...
function BrandPanel() {
  return (
    <div style={{ flex: "1 1 48%", position: "relative", overflow: "hidden", color: "var(--cream)",
      ...
      {/* forensic watermark — faint mono ledger lines */}
